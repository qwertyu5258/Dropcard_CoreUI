...
@Component({
  selector: 'my-card4',
  templateUrl: './my-card4.component.html',
  styleUrls: ['./my-card4.component.scss']
})

export class MyCard4Component implements OnInit, OnDestroy {
  ...
  onResize(event) {
    ...
    if (event.target.innerWidth > 1326) {
      ...
      // this.frontChartOptions.scales.xAxes[0].display = false
      // this.frontChartOptions.scales.yAxes[0].display = false
      // this.frontChartOptions.scales.yAxes[1].display = false

      this.elementRef.nativeElement.querySelector('.header').style.display = "none";
      this.elementRef.nativeElement.querySelector('.content').style.top = "0px";
      this.elementRef.nativeElement.querySelector('.content').style.height = "140px";
      this.elementRef.nativeElement.querySelector('.content').style.padding = "10px";
      this.elementRef.nativeElement.querySelector('#ml_value').style.display = "none";
      this.elementRef.nativeElement.querySelector('#mlh_value').style.display = "none";

      this.elementRef.nativeElement.querySelector('.content').style.backgroundColor = 'rgba(42, 168, 212, 1)'
      // this.frontChartColours[0].borderColor =  'rgb(255,255,255)'
      this.thumbnail = true
      this.frontChart.chart.update()
    }
  }
  ...
  zIndexChange(event) {
    ...
    if (event.path[5] && event.path[5].tagName == 'MY-CARD4') {
      event.path[5].style.zIndex = this.favoriteDevicesService.zIndex++
    } else if (event.path[6] && event.path[6].tagName == 'MY-CARD4') {
